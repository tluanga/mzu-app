...
from .model import ProgrammeCategory, UpdateProgrammeCategory
import re
router = APIRouter()

dbPath = 'programme_category'

# -----------------ADD NEW-----------


@router.post("/",
             response_description="Add a new Programme Category",
             response_model=ProgrammeCategory)
async def create(request: Request, programme_category: ProgrammeCategory = Body(...)):

    json_data = jsonable_encoder(programme_category)
...
# ----------GET ONE ITEM-----------
@router.get('/{id}', response_model=ProgrammeCategory)
async def get(request: Request, id: str):
    if(response := await request.app.mongodb[dbPath].find_one({"_id": id})) is not None:
        return response

    raise HTTPException(
        status_code=404, detail=f"Programmme Category {id} not found")

# --------Search Item


@router.get('/', response_model=List[ProgrammeCategory])
...
@ router.put("/{id}", response_model=ProgrammeCategory)
async def update(
    request: Request,
    id: str,
        programme_category: UpdateProgrammeCategory = Body(...)):
    ...
    raise HTTPException(
        status_code=404, detail=f"Category of id:{id} not found")
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f" Programme Category of {id} not found")
