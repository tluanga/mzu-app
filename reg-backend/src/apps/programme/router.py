...
from .model import Programme, UpdateProgramme
import re
router = APIRouter()
...
@router.post("/",
             response_description="Add a new programme",
             response_model=Programme)
async def create(request: Request, programme: Programme = Body(...)):

    json_data = jsonable_encoder(programme)
...
# ----------GET ONE ITEM-----------
@router.get('/{id}', response_model=Programme)
async def get(request: Request, id: str):
    if(response := await request.app.mongodb[dbPath].find_one({"_id": id})) is not None:
        return response

    raise HTTPException(
        status_code=404, detail=f"Programme {id} not found")

# --------Search Item


@router.get('/', response_model=List[Programme])
...
@ router.put("/{id}", response_model=Programme)
async def update(
    request: Request,
    id: str,
        programme: UpdateProgramme = Body(...)):
    # Check and remove empty key value
    _payload = {k: v for k,
                v in programme.dict().items() if v is not None}
    ...
    raise HTTPException(
        status_code=404, detail=f"Programme of id:{id} not found")
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"Programme of {id} not found")
