...
from .model import PersonCategory, UpdatePersonCategory
import re
router = APIRouter()

dbPath = 'person_category'
...
@router.post("/",
             ...
             response_model=PersonCategory)
async def create(request: Request, person_category: PersonCategory = Body(...)):

    json_data = jsonable_encoder(person_category)
...
# ----------GET ONE ITEM-----------
@router.get('/{id}', response_model=PersonCategory)
async def get(request: Request, id: str):
    if(response := await request.app.mongodb[dbPath].find_one({"_id": id})) is not None:
        return response

    raise HTTPException(
        status_code=404, detail=f"Category {id} not found")

# --------Search Item


@router.get('/', response_model=List[PersonCategory])
...
@ router.put("/{id}", response_model=PersonCategory)
async def update(
    request: Request,
    id: str,
        category: UpdatePersonCategory = Body(...)):
    # Check and remove empty key value
    _payload = {k: v for k,
                v in category.dict().items() if v is not None}
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"Category of {id} not found")
