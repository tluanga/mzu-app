...
from .model import Department, UpdateDepartment
import re
router = APIRouter()
...
@router.post("/",
             response_description="Add a new department",
             response_model=Department)
async def create(request: Request, department: Department = Body(...)):

    json_data = jsonable_encoder(department)
...
async def get(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"Department {id} not found")
...
async def update(
    request: Request,
    id: str,
        department: UpdateDepartment = Body(...)):
    # Check and remove empty key value
    _payload = {k: v for k,
                v in department.dict().items() if v is not None}
    ...
    raise HTTPException(
        status_code=404, detail=f"Deparment of id:{id} not found")
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"Department of {id} not found")
