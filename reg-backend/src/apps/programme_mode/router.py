...
from .model import ProgrammeMode, UpdateProgrammeMode
import re
router = APIRouter()

dbPath = 'programme_mode'

# -----------------ADD NEW-----------


@router.post("/",
             response_description="Add a new Programme Mode",
             response_model=ProgrammeMode)
async def create(request: Request, payload: ProgrammeMode = Body(...)):
...
# ----------GET ONE ITEM-----------
@router.get('/{id}', response_model=ProgrammeMode)
async def get(request: Request, id: str):
    if(response := await request.app.mongodb[dbPath].find_one({"_id": id})) is not None:
        return response

    raise HTTPException(
        status_code=404, detail=f"Programmme Mode {id} not found")

# --------Search Item


@router.get('/', response_model=List[ProgrammeMode])
...
@ router.put("/{id}", response_model=ProgrammeMode)
async def update(
    request: Request,
    id: str,
        programme_category: UpdateProgrammeMode = Body(...)):
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f" Programme Mode of {id} not found")
