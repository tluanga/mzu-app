...
from .model import UpdateUser, User
import re
router = APIRouter()
...
async def get(request: Request, id: str):
    if(user := await request.app.mongodb[dbPath].find_one({"_id": id})) is not None:
        return user
...
async def update(
    ...
    raise HTTPException(
        status_code=404, detail=f"Update of id:{id} not found")
...
async def delete(request: Request, id: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"User of {id} not found")
